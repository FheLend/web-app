...
export function useThemeStyles() {
  ...
  return {
    ...
    heroContainer: cn(
      ...
      isDark ? "bg-cryptic-dark" : "bg-gradient-to-b from-accent/5 to-accent/10"
    ),
    glowText: cn("text-cryptic-highlight", isDark ? "animate-glow" : ""),
    headingText: cn(isDark ? "text-glow" : "text-foreground"),
    cardStyles: cn(
      "cryptic-shadow",
      isDark
    ...
    tableHeader: cn(isDark ? "bg-cryptic-darker" : "bg-slate-50"),
    tableBody: cn(
      "divide-y",
